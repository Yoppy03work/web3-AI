...
- `/` — 最新ダイジェスト。タグでクライアントサイドフィルタ。媒体に 論文/研究 バッジ
- `/article/[id]` — 記事詳細。要約 / なぜ重要 / **本文の日本語訳**（失敗時は原文）/ 原文リンク
...
- **本文抽出は best-effort**: regex で `<article>` / `<main>` / `<p>` を拾うだけ。
  サイトによっては抜けが多い。Readability 移植で精度を上げられるが依存が増える。
- **タグ付けはキーワード一致**: 取りこぼし・誤爆あり。Claude の要約 1 リクエスト内で
  タグも返させると精度が上がる（バッチに `tags` を追加するだけ）。
- **全文検索**: `articles` が SQL に入ったので、Turso の FTS5 仮想テーブルを足すか、
  `WHERE title LIKE` から始められる。
- **読書ログ / 通知 / Notion 連携**: `articles` に `read_at` 等の列を足し、cron 後に
  Webhook（Discord/Slack）へ投げる経路を追加するのが次の自然な一手。
